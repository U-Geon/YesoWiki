...
- [x] `package.json`의 빌드 스크립트 수정 (`prisma generate` 추가)
- [x] `vercel.json` 구성 (빌드 캐시 및 브랜치 설정 제어)
- [x] Vercel 배포 시 필요한 환경변수 및 가이드 작성

## 수락 조건 (Acceptance Criteria)

- [x] Vercel 배포 시 Prisma Client 에러(`@prisma/client` not found 등)가 발생하지 않아야 함
- [x] PR이나 커밋 내용이 문서(`docs/`, `*.md`)만 변경된 경우 Vercel 빌드를 건너뛰어(Skip) 빌드 시간을 절약해야 함
