...
- [x] `.coderabbit.yaml` 파일 작성 (리뷰 언어, 프로필, path-instructions 등)
- [x] Next.js App Router 패턴 및 CLAUDE.md 컨벤션 리뷰 지침 등록
- [x] `plans/`, `docs/`, `.agents/` 등 문서 경로 리뷰 필터 설정

## 수락 조건 (Acceptance Criteria)

- [x] PR 생성 시 CodeRabbit이 한국어로 리뷰 코멘트를 남겨야 함
- [x] `src/` 코드 변경 시 Next.js RSC 경계, SSOT 원칙 등을 CodeRabbit이 검증해야 함
- [x] `plans/`, `docs/`, `.agents/` 변경 시 코드 리뷰를 건너뜀
